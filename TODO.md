...
- Gmail 使用 `https://mail.google.com/` 受限 scope；Google Cloud 项目已切换为 In Production，但受限 scope 验证仍待提交和通过。
- 当前 release workflow 会自动公开三平台产物，但 Windows 未签名，macOS 只是 ad-hoc 签名且未公证，Linux 也没有发行签名。
- 没有应用内安全更新通道；发布后无法便捷、可靠地给用户修复安全问题。
- 公开的产品主页、隐私政策、服务条款、支持页和数据删除说明已准备；仓库级 LICENSE 和 SECURITY 仍缺少。
- Gmail 移除流程已支持 Google token 撤销，并可选择删除该账户的 SQLite/WAL/SHM、通知基线、账户头像和账户级收藏；仍需使用真实 Gmail 账户完成实机验收。
...
- [x] **[P0]** 准备可公开访问且无需登录的 HTTPS 产品主页，清楚说明 Mine Mail 的用途和为什么需要 Gmail 邮件权限。
- [x] **[P0]** 准备同一已验证域名下的隐私政策、服务条款、支持页面和用户数据删除说明。
- [x] **[P0]** 在 Google OAuth 品牌配置中保持应用名称、狐狸图标、主页、隐私政策、支持邮箱和发行包品牌一致。
- [x] **[P0]** 完成域名所有权验证；不要依赖短链接、会跳转到无关域名的链接或需要登录才能查看的页面。
- [x] **[P0]** 隐私政策单独说明 Google 用户数据，并加入 Google API Services User Data Policy / Limited Use 的合规声明。
- [ ] **[P0]** 清楚描述：
...
- [x] **[P0]** “移除 Gmail 账户”时调用 Google token revocation endpoint，不能只删除本机 keyring 中的 refresh token。
- [x] **[P0]** 给用户提供明确的“断开并删除本地数据”动作，删除：
  - 该账户凭据和 OAuth token
  - 该账户邮件 SQLite 数据库及 WAL/SHM
  - 通知基线和账户级头像
  - 账户级收藏及其他派生数据
  - 无法安全重试的本地草稿/Outbox 前必须二次确认
- [x] **[P0]** 将“仅断开”“删除本地缓存”“撤销 Google 授权”的差异写清楚；执行失败时不能向用户显示已经全部删除。
- [x] **[P1]** 提供删除整个 Mine Mail 本地数据的入口和无需应用也能执行的支持说明。
...
- [x] **[P0]** 发布公开 `PRIVACY.md` 或网站隐私政策，内容与真实代码和默认设置一致。
