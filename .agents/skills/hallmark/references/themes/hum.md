...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `hum`. The palette + font stack live in the upstream `site/css/tokens.css` source under `[data-theme="hum"]`; that source is not bundled with this installed skill. This file carries signature moves, motion direction, voice fixtures, anti-patterns, and how Hum holds its identity against neighbouring themes. Hum is the catalog's only playful theme — its closest siblings are Coral (single-accent restraint, modern-minimal) and Bloom (warm-light atmospheric), not another playful register.
