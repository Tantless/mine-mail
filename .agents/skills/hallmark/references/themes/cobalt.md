...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `cobalt`. The OKLCH palette + font stack live in the upstream `site/css/tokens.css` source under `[data-theme="cobalt"]`. Canonical upstream build: `site/examples/cobalt-01/` (an API product landing with a request/response hero). These upstream files are not bundled with this installed skill.
...
Plus the Space Grotesk + Inter + JetBrains Mono link and the small reveal/type-in script. The canonical upstream build to mirror is `site/examples/cobalt-01/`, which is not bundled with this installed skill.
