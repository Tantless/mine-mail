...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `lumen`. Palettes + font stack live in the upstream `site/css/tokens.css` source under `[data-theme="lumen"]` (Night, default) and `[data-theme="lumen"][data-drop="day"]` (Day); that source is not bundled with this installed skill. This file carries the drops + apparatus family + signature moves + motion + anti-patterns.
