...
- Contacts are local-first. The **通讯录** workspace derives correspondents from cached message headers and offers one durable organization action: **收藏**. Favorites are keyed globally by normalized email, appear first in **全部** with a quiet tinted surface, and their correspondence view defaults to the active account. There is no separate save-contact state or local display-name editor. IMAP never owns contact synchronization. Provider contact sync remains a separate opt-in adapter layer (Google People API, Microsoft Graph, or CardDAV), while 163 uses local favorites plus import/export unless an official public sync interface becomes available.
