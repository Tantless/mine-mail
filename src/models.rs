...
impl ConnectionFailure {
    pub const fn new(protocol: ConnectionProtocol, kind: ConnectionFailureKind) -> Self {
        Self {
            protocol,
            kind,
            status_code: None,
    ...
    pub const fn with_status_code(mut self, status_code: Option<u16>) -> Self {
...
impl std::fmt::Display for ConnectionFailure {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(status_code) = self.status_code {
            write!(formatter, " with status {status_code}")?;
        }
        Ok(())
    }
}
...
pub struct ConnectionReport {
    ...
    pub smtp_ok: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub imap_failure: Option<ConnectionFailure>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub smtp_failure: Option<ConnectionFailure>,
}
