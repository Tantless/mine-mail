...
- 写工具只修改本轮 Agent 的 Rust 内存工作副本。整轮成功后 Rust 返回短请求标识
  和完整结果。邮件生成与自动模式由 React 使用本地内容指纹防止慢响应覆盖新编辑；
  独立优化保留点击时的正文快照，先显示可编辑的左右对比，只有用户明确选择并确认
  后才覆盖正文。失败时不应用部分结果。
- 工具参数和结果必须在 Rust 边界再次校验。邮箱地址、文本长度、HTML、附件类型
  和附件大小不能只依赖模型或前端保证。
- 工具调用的原始参数和结果不写入日志，也不作为会话历史长期保存。日志只保留
  工具名、字段名、大小、耗时和成功或失败状态。
...
| `set_draft_stationery` | 修改草稿信纸 | 写入 | 仅预留协议 |
...
作用：修改当前内存工作副本的主题。Rust 负责去除控制字符并执行长度上限。
...
作用：修改信纸类型以及发送时是否携带 Mine Mail 信纸。该协议现在固定，但首批
Agent 不注册此工具，后续获得产品确认后再按模块开放。
