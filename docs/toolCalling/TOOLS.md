...
- 聊天初始只开放读取工具和 `enable_generation`。后者成功后，Rust 只在当前用户
  轮次的下一次模型请求中改用生成工具白名单；同一批工具调用不能夹带写入，本轮
  完成、停止或失败后授权立即失效，前端模式选择器不会改变。
...
邮箱地址检索，供生成和自动模式选择真实联系人。模型不能把自己臆造的
邮箱地址冒充检索结果。
...
作用：聊天模式在用户当前消息已经明确要求生成或编辑邮件，或者明确接受助手上一轮
提出的具体生成思路时，调用此工具取得当前用户轮次的临时生成权限。输入为空。

```json
{}
```

成功后，下一次模型请求才会收到生成模式的读取与写入工具；调用本工具的同一批工具
调用仍按聊天白名单校验。授权不会切换前端模式，也不会进入 Session 或下一用户轮次。
临时生成写入前仍须在当前轮完成发信人、收件人、主题、正文、引用邮件和附件元数据
六项读取。没有明确用户授权时不得调用。
...
作用：修改当前内存工作副本的主题。生成、自动和临时提权的聊天模式注册此工具；
...
作用：修改信纸类型以及发送时是否携带 Mine Mail 信纸。生成、自动和临时提权的
聊天模式注册此工具；普通聊天和独立优化不注册。
