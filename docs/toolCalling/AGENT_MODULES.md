...
- MiMo 的 OpenAI 兼容接口目前只提供自动工具选择，不能可靠强制指定某个函数。独立优化
  需要正文和主题两项必读时，Rust 会在首个 Provider 请求前通过同一白名单工具边界读取
  点击时工作副本，并把标准 assistant tool call 与 tool result 历史交给模型；后续写入仍
  接受原有读前置校验，正文和主题不会绕过工具结果直接拼入初始 User Prompt。
...
Rust 在首个模型请求前通过受限工具读取完整正文和主题，并把标准工具调用历史交给模型；
工具执行边界仍强制任一写入必须已获得两项读取结果。主题为空时根据完整正文自动生成；非空主题只在用户明确要求，或现有主题
