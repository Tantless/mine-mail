...
目标：作为通用只读对话助手处理模型能够回答的各种话题，并在涉及邮件时提供更强
的理解、分析、发散讨论和写作建议。一般问题不读取草稿；邮件问题按需读取最少且
相关的当前草稿、引用邮件或附件内容。它没有任何写工具，因此无法修改草稿。
...
最终格式：可流式显示的安全 Markdown，篇幅和结构随问题复杂度调整。聊天可以在
回答中给出完整邮件范例或多个候选版本，但无论是否流式都不能获得写权限，也不会
形成草稿提案。
